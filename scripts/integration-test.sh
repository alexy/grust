...
ALL_BACKENDS=(sail surreal falkor helix issundb ladybug lancedb cocoindex pggraph)
DOCKER_BACKENDS=(surreal falkor issundb ladybug lancedb cocoindex pggraph)
QUICK_BACKENDS=(issundb ladybug lancedb cocoindex)
...
Backends: sail, surreal, falkor, helix, issundb, ladybug, lancedb, cocoindex, pggraph

Profiles:
  quick   Local integration checks that do not need daemons: issundb, ladybug, lancedb, cocoindex
...
if [[ ${#BACKENDS[@]} -eq 0 ]]; then
  if [[ -n "$PROFILE" ]]; then
    ...
    BACKENDS=(${GRUST_INTEGRATION_BACKENDS:-sail surreal falkor helix issundb ladybug lancedb cocoindex pggraph})
  fi
fi
...
backend_kind() {
  case "$1" in
    issundb|ladybug|lancedb|cocoindex)
      echo "local"
      ;;
...
run_backend() {
  ...
  case "$backend" in
    ...
    helix)
      ...
      ;;
    issundb)
      cargo test -p grust-issundb -- --test-threads=1
      ;;
    ladybug)
