...
The current `0.7.1` line is the first version where I think the whole shape is
...
- `grust-ladybug` embeds LadybugDB through the Rust `lbug` crate, stores schema-backed graph tables without a daemon, and can register Arrow IPC node, relationship, and CSR tables for direct Cypher queries.
...
- Ladybug keeps embedded schema-backed graph tables and can query external Arrow IPC tables.
