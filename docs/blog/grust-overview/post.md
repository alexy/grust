...
The `0.8.2` line is the first point where I think the full idea is visible in
...
upserts, deletes, and backend-specific transaction semantics. The 0.8.2 line
...
subset and lowers it into `GraphMutationPlan`; `SailGraphStore` then executes
that plan through `GraphMutationStore`, staged Arrow values, Delta `MERGE
INTO`, typed-table mirror writes, and the same delete paths as ordinary Grust
mutations.
