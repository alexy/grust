...
fn config() -> SailConfig {
    SailConfig {
        ...
        warehouse_dir: PathBuf::from("/tmp/grust-sail/test/warehouse"),
        ..SailConfig::default()
    }
}
...
fn set_then_get_transcript_binds_one_session() {
    let config = config();
    let set = set_request(&config).expect("set request");
...
fn verification_requires_exact_session_and_warehouse() {
    ...
    verify_warehouse(&valid, &config, "server-session").expect("exact response accepted");
    ...
    assert!(verify_warehouse(&wrong_path, &config, "server-session").is_err());

    let wrong_session = ConfigResponse {
        server_side_session_id: "replacement-session".to_string(),
        ..valid
    };
    assert!(verify_warehouse(&wrong_session, &config, "server-session").is_err());
}
