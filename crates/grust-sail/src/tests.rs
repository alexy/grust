...
fn cypher_match_where_rejects_deferred_predicate_forms() {
    for cypher in [
        ...
        "MATCH (n:Person) WHERE NOT n.status = 'active' OR n.status = 'pending' SET n.archived = true",
...
#[test]
fn sail_cypher_match_where_in_or_equal_predicates_execute_on_memory_facade() {
    let store = MemoryGraphStore::new();
...
#[test]
fn sail_cypher_match_where_negated_or_predicates_execute_on_memory_facade() {
    let store = MemoryGraphStore::new();
...
#[test]
fn sail_cypher_match_where_or_predicates_execute_on_memory_facade() {
    let store = MemoryGraphStore::new();
