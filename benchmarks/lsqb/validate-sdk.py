#!/usr/bin/env python3
"""Audit the pinned Surreal SDK example cohort; never issue publication receipts.
...
SOURCE = '945dfa748b99e259f21973a1e86aaa339a028834'
CLIENT = 'sha256:cd4d6f49887ff706c9ecec4b85ba95cb64ad6c07110bf59b96eb99f40034b72d'
SERVER = 'sha256:2642fcb2045a82b967d41ef01b327e43cb97a82557717a33318332f4a07a95b2'
...
def validate(directory):
    ...
    require(invocation['publication_qualified'] is False and invocation['source_revision'] == SOURCE
            and invocation['client_image_id'] == CLIENT and invocation['backend'] == 'surreal-sdk', 'invocation identity differs')
    ...
    require(cell['backend'] == dict(name='surreal-sdk', adapter='grust-surreal', adapter_version='0.13.1',
        runner_image=CLIENT, runner_image_id=CLIENT, resource_components=2,
        service_version='3.2.4+20260803.93ab219', image=f'surrealdb/surrealdb:v3.2.4@{SERVER}', image_id=SERVER),
    ...
    for query in cell['queries']:
        ...
        require(query['execution'] == {
            ...
            'transport': 'SurrealDB Rust SDK / WebSocket'}, 'execution plan differs')
        ...
        for phase, key, count in [('warmup', 'warmups', 2), ('measurement', 'measurements', 10)]:
            ...
            for iteration, sample in enumerate(query[key], 1):
                ...
                expected.append(dict(backend='surreal-sdk', complete=False, event='observation-recorded',
    ...
    for role, image in [('client', CLIENT), ('server', SERVER)]:
        ...
        if role == 'client':
            ...
            require(before['labels']['org.opencontainers.image.revision'] == SOURCE
                    and before['labels']['io.adversarial.grust.benchmark-feature'] == 'surreal', 'client feature/source differs')
        else:
            ...
            require(before['labels']['io.adversarial.disposable'] == 'surreal-sdk', 'server ownership differs')
