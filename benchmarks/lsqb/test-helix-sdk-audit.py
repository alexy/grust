...
spec = importlib.util.spec_from_file_location('audit', Path(__file__).with_name('validate-helix-sdk.py'))
audit = importlib.util.module_from_spec(spec)
spec.loader.exec_module(audit)
FIXTURE = os.environ.get('GRUST_HELIX_SDK_AUDIT_FIXTURE')


@unittest.skipUnless(FIXTURE, 'set GRUST_HELIX_SDK_AUDIT_FIXTURE to retained Docker evidence')
class AuditTests(unittest.TestCase):
    ...
    def test_rejects_mutated_records(self):
        mutations = [
            ('invocation.json', lambda r: r.update(server_source_revision='0' * 40)),
            ('server-before.json', lambda r: r['labels'].update({'org.opencontainers.image.revision': '0' * 40})),
            ...
            ('component.json', lambda r: r['backends'][0]['backend'].update(name='helix')),
            ...
            ('server-before.json', lambda r: r['labels'].update({'io.adversarial.disposable': 'helix'})),
